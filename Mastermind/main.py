...
""""
Wat algemene dingen die je wss zelf al wou doen:
1. Always try to explain yourself in code.
2. Don't comment out code. Just remove.
Uit https://gist.github.com/wojteklu/73c6914cc446146b8b533c0988cf8d29
...
"""Slimme manier om de game mode aan te roepen!"""
...
def selectGameOptions():
    checkInputOptions(input(
        "Do want to continue with normal game options?\n8 max tries?\n6 different colors?(wip)\nCode length of 4?(wip)\ny/n?"),
                      {'n': gameOptions, 'no': gameOptions}, selectGameOptions)
...
def AIvsAI():
    wins = 0
    loses = 0
    """Een beetje overbodige feedback, maar je gebruikt op andere plekken wel exceptions (zoals ValueError). Misschien een simpele toevoeging om dat dan op andere plekken waar dit kan ook te doen"""
    amountOfGames = input("Howmany games do you want to play?\n")
    game = 0
    print(f'Secret: {colors.SECRET}')
    """Kan een ValueError veroorzaken"""
...
def inputToColor(i) -> str:
    try:
        ...
        for color in colors.all_colors:
            """"Waar komt de lower functie vandaan?"""
